...
import { ArrowLeft, Edit, PlusCircle, FileText, CalendarDays, UserCircle2, Phone, Mail, MapPin, Briefcase, Users, Shield, Stethoscope, StickyNote, ShoppingBag, Repeat, Eye as EyeIcon } from 'lucide-react';
...
import { getOrdersByPatientId } from '@/services/order-service';
  ...
  useEffect(() => {
    if (patientId) {
      ...
      getPatientById(patientId).then(data => {
        ...
        if (data) {
          getOrdersByPatientId(data.id).then(orders => {
        ...
        <CardContent className="p-6 space-y-6">
            ...
            <section>
              <h3 className="text-xl font-semibold text-primary mb-3 flex items-center gap-2">
                <ShoppingBag className="h-5 w-5" /> Historial de Compras (Facturas)
              </h3>
              {isLoadingHistory ? <Skeleton className="h-24 w-full" /> : purchaseHistory.length > 0 ? (
                    ...
                    <TableBody>
                      {purchaseHistory.map((order) => (
                        <TableRow key={order.id}>
                          ...
                          <TableCell className="text-xs">{order.items.map(i => `${i.product.name} (x${i.quantity})`).join(', ')}</TableCell>
                          <TableCell className="text-right">{formatCurrencyCRC(order.totalCRC)}</TableCell>
                          <TableCell className="text-right">
                            <Button variant="outline" size="xs" onClick={() => handleViewInvoice(order)}>
